...
async function processEquipmentReport(userMessage, userId) {
    try {
        ...
        // 查詢設備的上次保養紀錄
        const query = "SELECT last_maintenance_time, last_maintenance_hours FROM equipment WHERE equipment_id = ?";
        ...
        // 計算距離上次保養的時數
        let hoursSinceLastMaintenance = runtimeHours - lastMaintenanceHours;
	...
	const updateQuery = `
            ...
            WHERE 設備編號 = ?
        `;
        await db.query(updateQuery, [status, runtimeHours, date, location, userId, equipmentId]);
...
if (userMessage.startsWith("更新設備 ")) {
    ...
    try {
        ...
        if (time) {
            updateFields.push("時間 = ?");
            updateValues.push(time);
        }
        if (location) {
            updateFields.push("地點 = ?");
...
if (userMessage.startsWith("移除設備 ")) {
        ...
        if (match) {
            ...
            try {
                ...
                const [rows] = await db.query(
                    "SELECT 上次保養時間, 上次保養時數 FROM 設備資料表 WHERE 設備編號 = ?",
                    [equipmentId]
                );
    ...
    try {
        ...
        // 更新設備的上次保養時間與運轉時數
        await db.query(
            "UPDATE 設備資料表 SET 上次保養時間 = ?, 上次保養時數 = ? WHERE 設備編號 = ?",
...
if (status === "更換第一道柴油") {
    replyMessage += `\n✅ **更換第一道柴油** 完成，提醒已解除。`;
                ...
                console.error("❌ 設備回報錯誤:", error);
                return replyToUser(event.replyToken, "⚠️ 設備回報失敗，請稍後再試。");
            }
        }
